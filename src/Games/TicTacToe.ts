...
export class TicTacToe implements IGame {
            ...
            board.addEventListener('click', (event) => {
                ...
                if (cell && cell.innerHTML === '') {
                    if (this.player1.isTurn) {
                        cell.innerHTML = this.player1.symbol;
                        
                        const player1i = this.calulateI(parseInt(cell.id), this.game.gridSize);
                        const player1j = this.calulateJ(parseInt(cell.id), this.game.gridSize);
                        for (let i = 0; i < this.game.board.length; i++) {
                            for (let j = 0; j < this.game.board[i].length; j++) {
                                if (i === player1i && j === player1j) {
                                    this.game.board[i][j] = this.player1.symbol;
                                }
                            }
                        }
                        ...
                        cell.innerHTML = this.player2.symbol;
                        const player2i = this.calulateI(parseInt(cell.id), this.game.gridSize);
                        const player2j = this.calulateJ(parseInt(cell.id), this.game.gridSize);
                        for (let i = 0; i < this.game.board.length; i++) {
                            for (let j = 0; j < this.game.board[i].length; j++) {
                                if (i === player2i && j === player2j) {
                                    this.game.board[i][j] = this.player2.symbol;
                                }
                            }
                        }
    ...
    private checkWinner(board: string[][], player1: any, player2: any) {
        ...
        for (let i = 0; i < board.length; i++) {
            ...
            let isGameOver = this.makeWinner(isWinnerPlayer1, isWinnerPlayer2, this.game);
        ...
        for (let i = 0; i < board.length; i++) {
            for (let j = 0; j < board[i].length; j++) {
                if (board[i][j] == player1.symbol && (i + j) === (this.game.gridSize - 1)) { isWinnerPlayer1++; }
                if (board[i][j] == player2.symbol && (i + j) === (this.game.gridSize - 1)) { isWinnerPlayer2++; }
            }
            let isGameOver = this.makeWinner(isWinnerPlayer1, isWinnerPlayer2, this.game);
            if (isGameOver) { return; }
        }
    ...
    private makeWinner(isWinnerPlayer1: number, isWinnerPlayer2: number, game: any): boolean {
    ...
    private resetGame(): void {
        ...
        this.game.board = Array(this.game.gridSize).fill([]).map(() => Array(this.game.gridSize).fill(''));
        this.player1.isTurn = true;
        this.player2.isTurn = false;
